...
def social_section(site_dir: str) -> list[str]:
    post = social.draft(site_dir)
    out = ["## Brand post this week"]
    if not post:
        return out + ["No field gained enough new nearby listings for a post.", ""]
    sent = [name for name, needs, _ in social.CHANNELS if all(os.environ.get(k) for k in needs)]
    how = (f"`growth/social.py` posts one like it to {', '.join(sent)} on Tuesdays and Thursdays."
           if sent else "No brand account is connected, so this is a draft to post by hand (see growth/social.py).")
    return out + [how, "", "```text", post[0], "```", ""]
...
def main():
    ...
    for section in (lambda: visits_section(now), demand_section, lambda: audience_section(site_dir),
                    ...
                    lambda: social_section(site_dir)):
        try:
            lines += section()
