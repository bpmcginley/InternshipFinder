...
| Host permission `<all_urls>` | Internship applications are hosted on thousands of different employer and applicant-tracking domains (Workday tenants, Greenhouse, Lever, iCIMS, Taleo, company career sites and more). The student can also paste any application link. The extension can't know these domains in advance, so it needs access to the page the student queued in order to read the form and fill it. It only acts on tabs it opened for an application the student chose. It doesn't read or change other browsing. |
